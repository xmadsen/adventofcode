...
register = {}
maxx = 0
def compare(cond_reg, comparison, cond_val):
	if cond_reg not in register.keys():
		register[cond_reg] = 0

	if comparison == '>':
		return(register[cond_reg] > cond_val)
	elif comparison == '<':
		return(register[cond_reg] < cond_val)
	elif comparison == '>=':
		return(register[cond_reg] >= cond_val)
...
for line in instring.split('\n'):
	...
	reg = linestring[0]
	plusminus = linestring[1]
	amt = int(linestring[2])
	cond_reg = linestring[4]
	comparison = linestring[5]
	cond_val = int(linestring[6])

	if plusminus == 'dec':
		amt *= -1

	if reg not in register.keys():
		register[reg] = 0
	if compare(cond_reg, comparison, cond_val):
		register[reg] += amt
		if register[reg] > maxx:
			maxx = register[reg]

print("Part 1 solution :", max(register.values()))
print("Part 2 solution :", maxx)