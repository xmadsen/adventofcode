...
# on the keypad: U moves up, D moves down, L moves left, and R moves
# right. Each line of instructions corresponds to one button, starting at
# the previous button (or, for the first line, the "5" button); press
# whatever button you're on at the end of each line. If a move doesn't
...
instructions = []
with open('day2_2016_input.txt') as file:
    for line in file.read().split('\n'):
        if line:
            instructions.append(line)


def do_operations(instring, problem):

    if problem == 1:
        keypad = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
        pos = [1, 1]
        for direction in instring:
            if direction == 'U':
                if pos[0] > 0:
                    pos[0] -= 1
            elif direction == 'D':
...
for instruction in instructions:
    passcode1 += str(do_operations(instruction, problem=1))
    passcode2 += str(do_operations(instruction, problem=2))

print("Part 1 solution :", passcode1)
print("Part 2 solution :", passcode2)
