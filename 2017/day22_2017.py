...
# wakes up, does some work, and goes back to sleep. The following steps
# are all executed in order one time each burst:
...
# After a total of 10000 bursts of activity, 5587 bursts will have caused
# an infection.
...
grid = [list(row) for row in instring.split('\n')]
carrier = [len(grid) // 2, len(grid[0]) // 2]
...
def burst():
    ...
    # ROTATE, THEN CLEAN OR INFECT
    if not is_part2:
        if grid[carrier[0]][carrier[1]] == '#':
            dir = rotate(dir, 'right')
            grid[carrier[0]][carrier[1]] = '.'
        else:
            dir = rotate(dir, 'left')
            grid[carrier[0]][carrier[1]] = '#'
            infectcount += 1
        ...
        if grid[carrier[0]][carrier[1]] == '.':
            dir = rotate(dir, 'left')
            grid[carrier[0]][carrier[1]] = 'W'
        elif grid[carrier[0]][carrier[1]] == 'W':
            # Do not change direction
            grid[carrier[0]][carrier[1]] = '#'
            infectcount2 += 1
        elif grid[carrier[0]][carrier[1]] == '#':
            dir = rotate(dir, 'right')
            grid[carrier[0]][carrier[1]] = 'F'

        elif grid[carrier[0]][carrier[1]] == 'F':
            dir = rotate(dir, 'left')
            dir = rotate(dir, 'left')
            grid[carrier[0]][carrier[1]] = '.'
    ...
    if carrier[0] not in range(len(grid)) or carrier[1] not in range(len(grid[0])):
        if carrier[0] not in range(len(grid)):
            if carrier[0] < 0:
                dotrow = list(['.'] * len(grid[0]))
                grid.insert(0, dotrow)
                carrier[0] = 0
            elif carrier[0] >= len(grid):
                dotrow = ['.'] * len(grid[0])
                grid.append(dotrow)
                carrier[0] = len(grid) - 1
        elif carrier[1] not in range(len(grid[0])):
            if carrier[1] < 0:
                for row in grid:
                    row.insert(0, '.')
                carrier[1] = 0
            elif carrier[1] >= len(grid[0]):
                for row in grid:
                    row.append('.')
                carrier[1] = len(grid[0]) - 1
...
def drawgrid(grid):
    for i, row in enumerate(grid):
        ...
        for j, el in enumerate(row):
            if i == carrier[0] and j == carrier[1]:
                print("\b[{}]".format(el), end='')
            else:
                print(el, end=' ')
        print('')
...
print("Part 1 solution :", infectcount)
...
print("Part 2 solution :", infectcount2)
