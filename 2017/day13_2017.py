...
for el in instring.split('\n'):
    lengths[int(el.split(':')[0])] = int(el.split(' ')[1])

penalty = 0


def penalty_for_pass(delay=0):
    penalty = 0

    for layer in lengths.keys():
        if (layer + delay) % (2 * lengths[layer] - 2) == 0:
            penalty += (layer + delay) * lengths[layer]

    return(penalty)

print("Part 1 solution :", penalty_for_pass(delay=0))
...
while penalty_for_pass(delay=delay) != 0:
    delay += 1
