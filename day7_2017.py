...
instring = """mypyemv (1058) -> tdssotr, pebnvks, zaulju
snslv (48)
aytbgf (30)
mkcrsy (42)
shwcd (42)
...
for line in instring.split('\n'):
	progs_above = []
	if '->' in line:
		progs_above = line.split('->')[1].split(' ')
		progs_above.remove('')
		progs_above = [prog.replace(',', '') for prog in progs_above]
	name = line.split(' ')[0]
	weight = int(line.split(' ')[1][1:-1])
	#print(progs_above, name, weight)
	programs[name] = {'weight' : weight, 'progs_above' : progs_above}
	#print(programs[name])

children = [val for val in [value['progs_above'] for key,value in programs.items()]]
children = [item for sublist in children for item in sublist]
all_progs = list(programs.keys())

for prog in set(children):
	all_progs.remove(prog)

print("Part 1 solution :", all_progs[0])
#print(programs.keys())
...
# for each parent, see if the set of the weights of the children is a single entry, then use bitwise xor to

# Lifted below solution from https://www.reddit.com/r/adventofcode/comments/7i44pg/2017_day_7_solutions/dqw0f0c/
...
for node in reversed(ordered):
	...
	if unbalanced:
		diff = abs(weights[unbalanced] - val)
		print('Part 2 solution :', graph.nodes[unbalanced]['weight'] - diff)
		break
