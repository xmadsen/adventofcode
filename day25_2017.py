...
with open('day25_input.txt') as intext:
    for state_lines in intext.read().split('\n\n'):
        # print(state_lines, end='\n\n')
        instructions[state_lines.split('\n')[
            0][-2:-1]] = [line.split('  ')[-1].split(' ')[-1][:-1] for line in state_lines.split('\n')[1:]]

instructions['F'] = instructions['F'][:-1]
...
def do_one_step():
    ...
    if cursor not in inftape.keys():
        inftape[cursor] = 0

    if inftape[cursor] == 0:
        inftape[cursor] = int(instructions[curr_state][1])
        if instructions[curr_state][2] == 'right':
            cursor += 1
        else:
            cursor -= 1
        curr_state = instructions[curr_state][3]
    elif inftape[cursor] == 1:
        inftape[cursor] = int(instructions[curr_state][5])
        if instructions[curr_state][6] == 'right':
            cursor += 1
        else:
            cursor -= 1
        curr_state = instructions[curr_state][7]

for step in range(steps):
    do_one_step()

checksum = sum(inftape.values())
